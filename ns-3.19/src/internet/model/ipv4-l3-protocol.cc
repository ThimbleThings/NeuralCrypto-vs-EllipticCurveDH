...
#include "../sim_insulatedsig/SKISUtils.h"
...
Ipv4L3Protocol::IpForward (Ptr<Ipv4Route> rtentry, Ptr<const Packet> p, const Ipv4Header &header)
{
  ...
  /// aur: Erweiterung iSig
  m_node->my_packet_filter.clear();
  //m_node->my_packet_filter.push_back("10.1.1.6");
  //m_node->my_packet_filter.push_back("10.1.1.7");
  char buff2[512];
  FILE *fp;
	std::ostringstream filter_name_ss;
	filter_name_ss << "/tmp/filter" << m_node->GetId();
	fp = fopen(filter_name_ss.str().c_str(), "r");
	if (fp != NULL) {		
    //std::cout << "Node " << m_node->GetId() << " reading filter file " << filter_name_ss.str() << "\n";
		while( !feof(fp) ) {
			//try to read 512 bytes from the stream, this operation is BLOCKING ...
     buff2[0] = '\0';
     fgets(buff2, 512 ,fp);
     buff2[strlen(buff2)-1] = '\0';
     //int mylen = strlen(buff2);
     //printf("%s, %d\n",buff2, mylen);
     if (strlen(buff2) != 0) m_node->my_packet_filter.push_back(buff2);
  	}
		pclose(fp);
   } 
  ...
  if (ipHeader.GetProtocol () == UdpL4Protocol::PROT_NUMBER ) {
      ...
      Ptr<Packet> udpPacket = p->Copy ();
      UdpHeader udpHeader;
		  udpPacket->RemoveHeader (udpHeader);		
			if(udpHeader.GetDestinationPort() == 4477 || udpHeader.GetDestinationPort() == 4478) { // filter only key mngt msg    
        std::vector<std::string>::iterator itr;
        for (itr = m_node->my_packet_filter.begin(); itr!= m_node->my_packet_filter.end(); ++itr) {
             Ipv4Address filterAddress((*itr).c_str());
             //std::cout << "--------- Node " << id << " filtering src addr " << *itr << "\n";
             if (srcAddress == filterAddress) {
                std::cout << "--------- Node " << id << " DISCARD packet UID=" << uid << " from " << srcAddress << " to " << destAddress << "\n";
                m_dropTrace (header, packet, DROP_ROUTE_ERROR, m_node->GetObject<Ipv4> (), interface);
                return;
             } 
          }
          //if (m_node->my_packet_filter.empty() == false) std::cout << "--------- Node " << id << " FORWARD packet UID=" << uid << " from " << srcAddress << " to " << destAddress << "\n";
      }
    }
  /// aur: Erweiterung iSig
