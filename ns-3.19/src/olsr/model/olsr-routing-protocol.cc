...
/// Erweiterung SKIS
void
RoutingProtocol::Dump2 (void)
{

  Time now = Simulator::Now ();
  std::cout << "Dumping for node with main address " << m_mainAddress << "\n";
  std::cout << " Neighbor set\n";
  for (NeighborSet::const_iterator iter = m_state.GetNeighbors ().begin (); 
       iter != m_state.GetNeighbors ().end (); iter++) {
      std::cout << "  " << *iter << "\n";
    }
   std::cout << " Neighbor set - SAVE\n";
   for (NeighborSet::const_iterator iter = m_state_save.GetNeighbors ().begin (); 
       iter != m_state_save.GetNeighbors ().end (); iter++) {
      std::cout << "  " << *iter << "\n";
    }
  /*
  std::cout << " Two-hop Neighbor set\n";
  for (TwoHopNeighborSet::const_iterator iter = m_state.GetTwoHopNeighbors ().begin ();
       iter != m_state.GetTwoHopNeighbors ().end (); iter++) {
      if (now < iter->expirationTime) { 
          std::cout << "  " << *iter << "\n";
  ...
  for (std::map<Ipv4Address, RoutingTableEntry>::const_iterator iter = m_table.begin (); iter != m_table.end (); iter++) {
      std::cout << "  dest=" << iter->first << ", next=" << iter->second.nextAddr 
                << ", viaIntf=" << iter->second.interface 
                << ", dist=" << iter->second.distance 
                << "\n";
    }
  std::cout << "\n";
  */
...
void
RoutingProtocol::SaveNeighborSet (void)
{
  uint32_t i = 0;  

  for (NeighborSet::const_iterator iter = m_state.GetNeighbors ().begin (); 
       iter != m_state.GetNeighbors ().end (); iter++) {
      SaveNeighborTuple(*iter);
      i++;
      //std::cout << "  " << *iter << "\n";
    }
  
  //std::cout << "Save " <<  i << " neighbors for node with main address " << m_mainAddress << "\n";
 
}

void
RoutingProtocol::SaveNeighborTuple (const NeighborTuple &tuple) {
  m_state_save.InsertNeighborTuple (tuple);
}


bool
RoutingProtocol::CompareNeighborSet (void)
{
  bool equal = false;
  int32_t not_found = 0;  
  int32_t found = 0; 
 
   //std::cout << "Compare for node with main address " << m_mainAddress << "\n";
  for (NeighborSet::const_iterator iter = m_state_save.GetNeighbors ().begin (); 
       iter != m_state_save.GetNeighbors ().end (); iter++) {
       if (iter->willingness != 0) { 
          if (m_state.FindNeighborTuple(iter->neighborMainAddr) == NULL) {
            not_found++;
          } else {
            found++;
          }
...
bool 
RoutingProtocol::CompareNeighborSet (int32_t in_num_equal) 
{
  ...
  if (in_num_equal <= -1) {
    //  all saved nodes found
    if (not_found == 0 && found > 0) {
    equal = true;
    }
  } else {
     // at minimum in_num_equal nodes are found  
     if (in_num_equal <= found) {
      equal = true;
    }
  }
